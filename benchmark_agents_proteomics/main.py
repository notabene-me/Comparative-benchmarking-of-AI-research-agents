...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        description="AI Agent Benchmark — untargeted proteomics comparison pipeline",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--input_dir",  default="./agent_outputs_proteomics",
                   help="Root directory containing agent output files.")
    p.add_argument("--output_dir", default="./benchmark_results_proteomics",
...
def run_pipeline(args) -> None:
    ...
    _save_csv(domain_scores,    tables_dir / "domain_proteomics_score_components.csv","domain components")
    ...
    audit_bundle = {
        ...
        "domain_proteomics_score_components": domain_scores,
...
def _create_demo_data(input_dir: Path) -> None:
    ...
    PROTEOMICS_PHRASES = [
        "Untargeted label-free proteomics analysis was performed using LC-MS/MS.",
        "Raw spectra were processed in MaxQuant with match-between-runs enabled.",
        "Quality control samples were injected every 10 samples.",
        "Data normalisation was performed using the pooled QC approach.",
        "Missing values were imputed using k-nearest neighbour imputation.",
        "Principal component analysis (PCA) revealed clear group separation.",
        "PLS-DA was applied to identify discriminant proteins.",
        "Statistical significance was assessed using limma on log2 intensities.",
        "Multiple testing correction was performed using Benjamini-Hochberg FDR.",
        "A total of 3421 proteins were identified at 1% FDR using Percolator.",
        "GO enrichment and KEGG pathway analysis were conducted on differential proteins.",
        "Fold change > 1.5 and FDR < 0.05 were applied as significance thresholds.",
        "Volcano plot visualization revealed 156 significantly altered proteins.",
        "Mitochondrial and ribosomal protein sets were significantly enriched.",
        "Batch correction was applied using ComBat algorithm.",
        "Log2 fold change (log2FC) was calculated for all quantified proteins.",
        "Results were visualised as a heatmap of the top 50 differential proteins.",
        "DDA acquisition was used with a 90-minute LC gradient.",
        "Protein inference was performed at the protein-group level in MaxQuant.",
        "Phosphorylation and PTM-related proteins were discussed in biological interpretation.",
        "Biomarker candidates were validated across all 8 independent runs.",
    ]
    ...
    for agent in agents:
        for run_num in range(1, n_runs + 1):
            ...
            writer.writerow(["protein", "pvalue", "fdr", "log2fc", "significant"])
            for i in range(random.randint(10, 30)):
                ...
                writer.writerow([f"PROT{i+1:05d}", pval, fdr, fc, sig])
