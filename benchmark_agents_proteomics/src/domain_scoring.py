"""
domain_scoring.py — domain-aware proteomics scoring.
...
def _c_proteomics_platform(r: dict) -> int:
    signals = (r.get("kw_LC-MS", 0) + r.get("kw_LC-MS/MS", 0)
               + r.get("kw_DDA", 0) + r.get("kw_DIA", 0)
               + r.get("kw_label_free", 0) + r.get("kw_untargeted_proteomics", 0))
...
def _c_significant_proteins(r: dict) -> int:
    sig = r.get("aud_significant", 0)
    prots = r.get("n_protein_mentions", 0)
    return _grade(sig == 0 and prots == 0, sig > 0 and prots <= 5, sig > 0 and prots > 5)
...
def _c_pathway_go(r: dict) -> int:
    pw = (r.get("kw_pathway_analysis", 0) + r.get("kw_KEGG", 0)
          ...
          + r.get("kw_Reactome", 0) + r.get("n_pathway_mentions", 0))
    func = (r.get("bio_kinase", 0) + r.get("bio_transcription_factor", 0)
            + r.get("bio_mitochondrial", 0) + r.get("bio_immune", 0)
            + r.get("bio_PTM", 0))
...
DOMAIN_CRITERIA: List = [
    ("dm_proteomics_platform",      _c_proteomics_platform),
    ...
    ("dm_significant_proteins",     _c_significant_proteins),
