...
DOMAIN_SAFEGUARD = (
    "Domain-aware scoring provides a proteomics-specific sensitivity analysis "
...
def _write_audit_section(audit: dict, h, p, ul, table, fig, run_scores) -> None:
    ...
    # Domain-aware
    h(3, "Domain-aware proteomics score")
    p(DOMAIN_SAFEGUARD)
    ...
    table(_df("domain_proteomics_score_components"))
...
def generate_report(
    ...
    p(
        ...
        f"({agent_list}) evaluated on an untargeted proteomics analysis task. "
    ...
    h(2, "2. Study Design")
    ul([
        "Each AI agent analysed the **same** untargeted proteomics dataset.",
        ...
        if "n_chars" in file_features.columns and len(file_features) > 1:
            ...
            if first_row.get("n_chars", 0) >= 0.5 * max_chars:
                p(
                    ...
                    "results table) that contains many rows of protein data — producing "
                ...
                p(
                    ...
                    "data tables with many protein entries. This is expected and not an error."
                )
    ...
    h(3, "Cross-Agent Reproducibility and Ranking of AI Research Agents on an Untargeted Proteomics Task")
    p(
        f"{n_agents} AI agents — {agent_list} — each independently analysed the same "
        "untargeted proteomics dataset and produced 8 runs of output. "
