...
METHODOLOGICAL_KEYWORDS: Dict[str, List[str]] = {
    ...
    "pooled_QC":            [r"\bpooled\s+QC\b"],
    "batch_correction":     [r"\bbatch\s+correction\b"],
    "normalization":        [r"\bnormali[sz]ation\b", r"\bnormali[sz]e[d]?\b"],
    "scaling":              [r"\bscaling\b", r"\bunit\s+variance\b", r"\bauv\b"],
    ...
    "STRING":               [r"\bSTRING\b"],
    "UniProt":              [r"\bUniProt\b"],
    "MaxQuant":             [r"\bMaxQuant\b"],
    "Proteome_Discoverer":  [r"\bProteome\s+Discoverer\b"],
    "Perseus":              [r"\bPerseus\b"],
    "MSstats":              [r"\bMSstats\b"],
    "limma":                [r"\blimma\b"],
    "Percolator":           [r"\bPercolator\b"],
    "LC-MS":                [r"\bLC[-\s]?MS\b"],
    "LC-MS/MS":             [r"\bLC[-\s]?MS/MS\b"],
    "DDA":                  [r"\bDDA\b", r"\bdata[-\s]dependent\s+acquisition\b"],
    "DIA":                  [r"\bDIA\b", r"\bdata[-\s]independent\s+acquisition\b"],
    "label_free":           [r"\blabel[-\s]free\b", r"\bLFQ\b", r"\bMaxLFQ\b"],
    "iTRAQ":                [r"\biTRAQ\b"],
    "TMT":                  [r"\bTMT\b", r"\btandem\s+mass\s+tag\b"],
    "SILAC":                [r"\bSILAC\b"],
    "peptide":              [r"\bpeptide[s]?\b"],
    "PSM":                  [r"\bPSM[s]?\b", r"\bpeptide[-\s]spectrum\s+match"],
    "protein_inference":    [r"\bprotein\s+inference\b", r"\bprotein\s+group[s]?\b"],
    "untargeted_proteomics": [r"\buntargeted\s+proteomics\b", r"\blabel[-\s]free\s+proteomics\b"],
}

BIOLOGICAL_TERMS: Dict[str, List[str]] = {
    "protein":              [r"\bprotein[s]?\b"],
    "peptide":              [r"\bpeptide[s]?\b"],
    "pathway":              [r"\bpathway[s]?\b"],
    "GO":                   [r"\bgene\s+ontology\b", r"\bGO\s+term[s]?\b"],
    "kinase":               [r"\bkinase[s]?\b"],
    "transcription_factor": [r"\btranscription\s+factor[s]?\b"],
    "extracellular_matrix": [r"\bextracellular\s+matrix\b", r"\bECM\b"],
    "mitochondrial":        [r"\bmitochondri(al|a)\b"],
    "ribosomal":            [r"\bribosomal\b", r"\bribosome[s]?\b"],
    ...
    "PTM":                  [r"\bPTM[s]?\b", r"\bpost[-\s]translational\s+modification"],
    "phosphorylation":      [r"\bphosphorylat"],
    "signaling":            [r"\bsignal(ing| transduction)\b"],
    "proteostasis":         [r"\bproteostasis\b", r"\bprotein\s+homeostasis\b"],
...
AUDIT_KEYWORDS: Dict[str, List[str]] = {
    "preprocessing":   [r"\bpre[-\s]?process", r"\bdata\s+cleaning\b", r"\bpeak\s+picking\b",
                        r"\bdeconvolution\b", r"\bfeature\s+detection\b",
                        r"\bprotein\s+inference\b", r"\bpeptide\s+identification\b",
                        r"\bspectral\s+library\b", r"\bsearch\s+engine\b"],
    "significant":     [r"\bsignificant", r"\bsignif\.", r"\bdifferentially\s+abundant\b"],
...
def extract_file_features(file_id: str, text: str) -> dict:
    ...
    features["n_protein_mentions"] = n_proteins
...
def proxy_depth_components(row: dict) -> dict:
    ...
    return {
        "D_protein_interpretation":
            int(row.get("n_protein_mentions", 0) > 5),
        "D_pathway_interpretation":
            int(row.get("n_pathway_mentions", 0) > 0 or row.get("kw_pathway_analysis", 0) > 0
                ...
                or row.get("kw_GSEA", 0) > 0 or row.get("kw_Reactome", 0) > 0),
        "D_protein_function_interpretation":
            int(row.get("bio_kinase", 0) > 0 or row.get("bio_transcription_factor", 0) > 0
                or row.get("bio_mitochondrial", 0) > 0 or row.get("bio_immune", 0) > 0
                or row.get("bio_PTM", 0) > 0 or row.get("bio_phosphorylation", 0) > 0),
