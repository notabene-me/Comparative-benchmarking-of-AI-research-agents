"""
domain_scoring.py — domain-aware metabolomics scoring.
...
# Ordered list of (criterion_name, grading_function).
# Each grading function takes a run-feature row dict and returns 0/1/2.
...
def _c_lcms_context(r: dict) -> int:
    signals = (r.get("kw_LC-MS", 0) + r.get("kw_LC-MS/MS", 0)
               + r.get("kw_HILIC", 0) + r.get("kw_untargeted_metabolomics", 0))
    return _grade(signals == 0, signals == 1, signals >= 2)


def _c_qc_strategy(r: dict) -> int:
    qc = r.get("kw_QC", 0)
    pooled = r.get("kw_pooled_QC", 0)
    return _grade(qc == 0 and pooled == 0, qc > 0 and pooled == 0, pooled > 0)


def _c_pooled_qc(r: dict) -> int:
    pooled = r.get("kw_pooled_QC", 0)
    qc = r.get("kw_QC", 0)
    return _grade(pooled == 0 and qc == 0, qc > 0 and pooled == 0, pooled > 0)


def _c_normalization_scaling(r: dict) -> int:
    norm = r.get("kw_normalization", 0)
    scal = r.get("kw_scaling", 0)
    return _grade(norm == 0 and scal == 0, (norm > 0) ^ (scal > 0), norm > 0 and scal > 0)


def _c_missing_values(r: dict) -> int:
    imp = r.get("kw_imputation", 0)
    return _grade(imp == 0, imp == 1, imp >= 2)
...
def _c_significant_features(r: dict) -> int:
    sig = r.get("aud_significant", 0)
    mets = r.get("n_metabolite_mentions", 0)
    return _grade(sig == 0 and mets == 0, sig > 0 and mets <= 5, sig > 0 and mets > 5)
...
def _c_pathway_class(r: dict) -> int:
    pw = r.get("kw_pathway_analysis", 0) + r.get("kw_KEGG", 0) + r.get("n_pathway_mentions", 0)
    cls = (r.get("bio_lipid", 0) + r.get("bio_amino_acid", 0)
           + r.get("bio_fatty_acid", 0) + r.get("bio_sphingolipid", 0))
    return _grade(pw == 0 and cls == 0, (pw > 0) ^ (cls > 0), pw > 0 and cls > 0)
...
DOMAIN_CRITERIA: List = [
    ("dm_lcms_context",            _c_lcms_context),
    ("dm_qc_strategy",             _c_qc_strategy),
    ("dm_pooled_qc",               _c_pooled_qc),
    ("dm_normalization_scaling",   _c_normalization_scaling),
    ("dm_missing_values",          _c_missing_values),
...
def compute_domain_scores(run_features: pd.DataFrame) -> pd.DataFrame:
    """
    Return one row per (agent, run) with each domain criterion (0/1/2),
    the raw sum, and the normalised DomainScore (0–100).
    """
