...
Metabolomics project: folder names are scoped to this repo only.
"""
...
# This project's default folders (macOS / Linux and Windows)
AGENT_INPUT_FOLDER = "agent_outputs_metabolomics"
BENCHMARK_OUTPUT_FOLDER = "benchmark_results_metabolomics"
AGENT_INPUT_FOLDER_WIN64 = "agent_outputs_metabolomics_Win64"
BENCHMARK_OUTPUT_FOLDER_WIN64 = "benchmark_results_metabolomics_Win64"
...
def is_ignored_file(path: Path) -> bool:
    """Return True for OS metadata and other non-agent artifacts."""
...
def is_ignored_dir(name: str) -> bool:
    """Return True for directories that should not be walked during scan."""
    lower = name.lower()
...
def resolve_agent_folder_name(dir_name: str) -> str | None:
    """Case-insensitive lookup for standard agent folder names."""
...
def ensure_benchmark_output_layout(
    ...
    """Ensure tables/, figures/, and report/ exist under *output_dir*."""
...
def configure_matplotlib_backend() -> None:
    """Use a non-interactive backend on Windows and headless environments."""
...
def create_project_folders(root: os.PathLike | str | None = None) -> List[Tuple[Path, bool]]:
    """
    Create this project's macOS and Win64 input/output folders under *root*.

    Returns list of (path, was_created).
    """
...
def create_win64_folders(root: os.PathLike | str | None = None) -> List[Tuple[Path, bool]]:
    """Create only the Win64 input/output folders for this metabolomics project."""
    base = Path(root or Path.cwd())
    return [
