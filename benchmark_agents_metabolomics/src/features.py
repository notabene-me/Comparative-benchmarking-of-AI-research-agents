...
METHODOLOGICAL_KEYWORDS: Dict[str, List[str]] = {
    ...
    "KEGG":                 [r"\bKEGG\b"],
    "HMDB":                 [r"\bHMDB\b"],
    "MetaboAnalyst":        [r"\bMetaboAnalyst\b"],
    "LC-MS":                [r"\bLC[-\s]?MS\b"],
    "LC-MS/MS":             [r"\bLC[-\s]?MS/MS\b"],
    "HILIC":                [r"\bHILIC\b"],
    "untargeted_metabolomics": [r"\buntargeted\s+metabolomics\b"],
}

BIOLOGICAL_TERMS: Dict[str, List[str]] = {
    "metabolite":           [r"\bmetabolite[s]?\b"],
    "pathway":              [r"\bpathway[s]?\b"],
    "lipid":                [r"\blipid[s]?\b"],
    "amino_acid":           [r"\bamino\s+acid[s]?\b"],
    "fatty_acid":           [r"\bfatty\s+acid[s]?\b"],
    "sphingolipid":         [r"\bsphingolipid[s]?\b"],
    "phospholipid":         [r"\bphospholipid[s]?\b"],
    "glycolysis":           [r"\bglycolysis\b"],
    "TCA_cycle":            [r"\bTCA\s+cycle\b", r"\bKrebs\s+cycle\b"],
...
def extract_file_features(file_id: str, text: str) -> dict:
    ...
    features["n_metabolite_mentions"] = n_metabolites
...
def proxy_depth_components(row: dict) -> dict:
    ...
    return {
        "D_metabolite_interpretation":
            int(row.get("n_metabolite_mentions", 0) > 5),
        "D_pathway_interpretation":
            int(row.get("n_pathway_mentions", 0) > 0 or row.get("kw_pathway_analysis", 0) > 0
                or row.get("kw_KEGG", 0) > 0 or row.get("kw_MetaboAnalyst", 0) > 0),
        "D_metabolite_class_interpretation":
            int(row.get("bio_lipid", 0) > 0 or row.get("bio_amino_acid", 0) > 0
                or row.get("bio_fatty_acid", 0) > 0 or row.get("bio_sphingolipid", 0) > 0
                or row.get("bio_phospholipid", 0) > 0),
